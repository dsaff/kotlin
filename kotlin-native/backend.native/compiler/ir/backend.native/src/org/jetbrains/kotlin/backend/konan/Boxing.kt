...
private val boxCacheGlobals = mutableMapOf<BoxCache, StaticData.Global>()
...
private fun initCache(cache: BoxCache, context: Context, cacheName: String,
    ...
    return if (declareOnly) {
        staticData.createGlobal(LLVMArrayType(llvmBoxType, end - start + 1)!!, cacheName, true).also {
            LLVMDumpValue(it.llvmGlobal)
        }
    } else {
...
internal fun IrConstantPrimitive.toBoxCacheValue(context: Context): ConstValue? {
    ...
    return if (value in start..end) {
        boxCacheGlobals[cacheType]?.pointer?.getElementPtr(value.toInt() - start)?.getElementPtr(0)
    } else {
        null
    }
}
