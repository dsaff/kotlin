...
internal typealias Kt1RendererConsumer = StringBuilder

internal class Kt1Renderer(
    private val analysisSession: Kt1AnalysisSession,
    ...
    private val typeRenderer = Kt1TypeRenderer(options.typeRendererOptions, isDebugText)
    ...
    private fun Kt1RendererConsumer.curlyBlock(body: Kt1RendererConsumer.() -> Unit) {
        append(" {")
        indentedBlock(body)
        appendLine()
        renderIndentation()
    ...
    private fun Kt1RendererConsumer.indentedBlock(body: Kt1RendererConsumer.() -> Unit) {
        indentation += 1
        body()
        indentation -= 1
    }

    fun render(descriptor: DeclarationDescriptor, consumer: Kt1RendererConsumer) {
        consumer.renderDeclaration(descriptor)
    }

    private fun Kt1RendererConsumer.renderType(type: KotlinType, shouldApproximate: Boolean = false) {
    ...
    private fun Kt1RendererConsumer.renderDeclaration(descriptor: DeclarationDescriptor) {
        when (descriptor) {
    ...
    private fun Kt1RendererConsumer.renderClassifier(descriptor: ClassifierDescriptor) {
        when (descriptor) {
    ...
    private fun Kt1RendererConsumer.renderTypeAlias(descriptor: TypeAliasDescriptor) {
    ...
    private fun Kt1RendererConsumer.renderTypeParameters(typeParameters: List<TypeParameterDescriptor>) {
        renderList(typeParameters, separator = ", ", prefix = "<", postfix = ">", renderWhenEmpty = false) { renderTypeParameter(it) }
    }

    private fun Kt1RendererConsumer.renderTypeParameter(descriptor: TypeParameterDescriptor) {
    ...
    private fun Kt1RendererConsumer.renderClass(descriptor: ClassDescriptor) {
        appendLine()
        renderIndentation()
    ...
    private fun <T : DeclarationDescriptor> sortDeclarations(declarations: List<T>): List<T> {
        ...
        return declarations.sortedWith(Comparator { left, right ->
            ...
            val leftString = StringBuilder().apply { renderDeclaration(left) }.toString()
            val rightString = StringBuilder().apply { renderDeclaration(right) }.toString()
            return@Comparator leftString.compareTo(rightString)
        })
    }

    private fun Kt1RendererConsumer.renderSupertypes(descriptor: ClassDescriptor) {
    ...
    private fun Kt1RendererConsumer.renderCallable(descriptor: CallableDescriptor) {
        when (descriptor) {
    ...
    private fun Kt1RendererConsumer.renderPropertyAccessor(descriptor: PropertyAccessorDescriptor) {
        appendLine()
        indentedBlock {
            renderIndentation()
            if (options.renderDeclarationHeader) {
    ...
    private fun Kt1RendererConsumer.renderEnumEntry(descriptor: ClassDescriptor) {
    ...
    private fun Kt1RendererConsumer.renderLocalVariable(descriptor: LocalVariableDescriptor) {
        appendLine()
        renderIndentation()
    ...
    private fun Kt1RendererConsumer.renderProperty(descriptor: PropertyDescriptor) {
        appendLine()
        renderIndentation()
    ...
    private fun Kt1RendererConsumer.renderReceiver(descriptor: CallableDescriptor) {
    ...
    private fun Kt1RendererConsumer.renderConstructor(descriptor: ConstructorDescriptor) {
        appendLine()
        renderIndentation()
        if (options.renderDeclarationHeader) {
            renderAnnotations(descriptor.annotations)
    ...
    private fun Kt1RendererConsumer.renderFunction(descriptor: FunctionDescriptor) {
        appendLine()
        renderIndentation()
    ...
    private fun Kt1RendererConsumer.renderValueParameters(valueParameters: List<ValueParameterDescriptor>) {
        renderList(valueParameters, separator = ", ", prefix = "(", postfix = ")", renderWhenEmpty = true) { renderValueParameter(it) }
    }

    private fun Kt1RendererConsumer.renderValueParameter(descriptor: ValueParameterDescriptor, name: Name = descriptor.name) {
        if (options.renderDeclarationHeader) {
            renderAnnotations(descriptor.annotations)
        }
    ...
    private fun Kt1RendererConsumer.renderLocalDeclarations(descriptor: CallableMemberDescriptor) {
        if (!options.renderContainingDeclarations) {
            return
        }
    ...
    private fun Kt1RendererConsumer.renderName(descriptor: DeclarationDescriptor) {
        append(descriptor.name.render())
    }

    private fun Kt1RendererConsumer.renderAnnotations(annotations: Annotations, predicate: (ClassId) -> Boolean = { true }) {
    ...
    private fun Kt1RendererConsumer.renderModifiers(descriptor: DeclarationDescriptor) {
    ...
    private fun Kt1RendererConsumer.renderVisibility(descriptor: DeclarationDescriptorWithVisibility) {
    ...
    private fun Kt1RendererConsumer.renderModality(descriptor: MemberDescriptor) {
    ...
    private fun Kt1RendererConsumer.renderModifier(text: String, state: Boolean, modifier: RendererModifier? = null) {
        if (state && (modifier == null || modifier in options.modifiers)) {
            append(text).append(' ')
        }
    }

    private fun Kt1RendererConsumer.renderIndentation() {
        append(INDENTATION.repeat(indentation))
    }
}

internal fun <T> Kt1RendererConsumer.renderList(
    list: Collection<T>,
    separator: String,
    prefix: String = "",
    postfix: String = "",
    renderWhenEmpty: Boolean = true,
    renderItem: Kt1RendererConsumer.(T) -> Unit
) {
