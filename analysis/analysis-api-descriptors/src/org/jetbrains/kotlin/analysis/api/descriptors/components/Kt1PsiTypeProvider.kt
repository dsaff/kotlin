...
import org.jetbrains.kotlin.analysis.api.descriptors.utils.Kt1JvmTypeMapper
...
internal class Kt1PsiTypeProvider(override val analysisSession: Kt1AnalysisSession) : KtPsiTypeProvider() {
    override val token: ValidityToken
        get() = analysisSession.token

    private val typeMapper by lazy { Kt1JvmTypeMapper(analysisSession.resolveSession) }

    override fun asPsiType(type: KtType, context: PsiElement, mode: TypeMappingMode): PsiType? = withValidityAssertion {
        val kotlinType = (type as Kt1Type).type
