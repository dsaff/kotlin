...
internal class Kt1TypeProvider(override val analysisSession: Kt1AnalysisSession) : KtTypeProvider() {
    ...
    override val builtinTypes: KtBuiltinTypes by cached { Kt1BuiltinTypes(analysisSession) }
    ...
    override fun commonSuperType(types: Collection<KtType>): KtType = withValidityAssertion {
        val kotlinTypes = types.map { (it as Kt1Type).type }
    ...
    override fun haveCommonSubtype(a: KtType, b: KtType): Boolean {
        return areTypesCompatible((a as Kt1Type).type, (b as Kt1Type).type)
    }
...
private class Kt1BuiltinTypes(private val analysisSession: Kt1AnalysisSession) : KtBuiltinTypes() {
    override val token: ValidityToken
        get() = analysisSession.token
