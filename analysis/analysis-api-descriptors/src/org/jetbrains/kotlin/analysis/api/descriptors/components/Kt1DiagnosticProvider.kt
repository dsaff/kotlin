...
internal class Kt1DiagnosticProvider(override val analysisSession: Kt1AnalysisSession) : KtDiagnosticProvider() {
    ...
    override fun getDiagnosticsForElement(element: KtElement, filter: KtDiagnosticCheckerFilter): Collection<KtDiagnosticWithPsi<*>> {
        withValidityAssertion {
            ...
            return diagnostics.map { Kt1Diagnostic(it, token) }
        }
    }
        ...
        withValidityAssertion {
            ...
            for (diagnostic in bindingContext.diagnostics) {
                if (diagnostic.psiFile == ktFile) {
                    result += Kt1Diagnostic(diagnostic, token)
                }
            }
            return result
        }
...
private class Kt1Diagnostic(private val diagnostic: Diagnostic, override val token: ValidityToken) : KtDiagnosticWithPsi<PsiElement> {
